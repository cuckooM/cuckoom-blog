...
tags:
  ...
  - Spring Boot
  - 双向认证
description: 详细介绍如何为 Postgresql 数据库配置 SSL 双向认证，并通过 Spring Boot 应用访问
...
> 注意："Common Name"应该写服务器的 IP 地址或域名。

#### 由于是自签名的，因此可以使用服务器证书作为受信任的根证书
...
> 注意："Common Name"应该设置为我们将连接的数据库用户名。

#### 转化格式。将 PEM 格式密钥转化为 DER 格式。
...
连接成功后，可以通过以下 SQL 验证 SSL 连接：

```sql
SELECT * FROM pg_stat_ssl;
```

如果连接使用了 SSL，会显示相应的 SSL 信息。
